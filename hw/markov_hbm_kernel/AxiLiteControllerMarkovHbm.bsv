package AxiLiteControllerMarkovHbm;

// ============================================================
// Pins interface — directly maps to AXI4-Lite signals
// ============================================================
interface AxiLiteMarkovHbmPinsIfc#(numeric type addrSz, numeric type dataSz);
...
// ============================================================
interface AxiLiteMarkovHbmIfc#(numeric type addrSz, numeric type dataSz);
	interface AxiLiteMarkovHbmPinsIfc#(addrSz, dataSz) pins;
	...
	// HBM DNA pointer (64-bit, set by host before start)
	method Bit#(32) dna_ptr_lo;
	method Bit#(32) dna_ptr_hi;

	// Number of DNA entries to read
	method Bit#(32) num_entries;
...
// BVI import
// ============================================================
import "BVI" s_axi_control_markov_hbm =
module mkAxiLiteControllerMarkovHbm#(Clock aclk, Reset arst) (AxiLiteMarkovHbmIfc#(addrSz, dataSz));
	...
	interface AxiLiteMarkovHbmPinsIfc pins;
	...
	method dna_ptr_lo dna_ptr_lo() clocked_by(aclk) reset_by(arst);
	method dna_ptr_hi dna_ptr_hi() clocked_by(aclk) reset_by(arst);
	method num_entries num_entries() clocked_by(aclk) reset_by(arst);
	...
	schedule (
		...
		interrupt,
		command, command_write_pulse,
		dna_ptr_lo, dna_ptr_hi, num_entries,
